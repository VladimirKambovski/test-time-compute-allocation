"""
Day 13/14 (pulled forward under time pressure): the REAL question behind
"is the controller useless because it always predicts STOP" -- does
Detective actually discriminate, under proper held-out evaluation, or
does it collapse to the majority class? The only number that exists so
far (93.6%/91.9%, HANDOFF/day11_e0_thin_slice.py) is an IN-SAMPLE fit on
the 100-problem dev slice -- optimistic by construction, explicitly
flagged in that script's own comments as "NOT final."

This runs the real thing: grouped 5-fold CV (grouped by problem --
trivial here since each problem contributes exactly one row -- stratified
by benchmark per docs/brief.md section 16's Protocol), full P1+P2 scale
(754 problems), out-of-fold macro-AUROC + per-class AUROC/recall/support,
compared against the majority-class trivial floor. Mirrors
DetectiveController's exact preprocessing (NaN imputed by TRAIN column
mean, StandardScaler, multinomial LogisticRegression) but implemented
directly over arrays for CV convenience rather than repeated single-shot
fits.

Memory-safe: streams pools one at a time to build the (features, label)
table, then does the CV fit/eval entirely in memory on that small table
(754 rows x 13 features -- trivial size).
"""
import gc
import json
import sys
...
def main():
    ...
    rows = []  # (benchmark_id, label, feature_dict)
    n_processed = 0
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, benchmarks)):
        ...
        # align columns to the global `classes` order (a fold might not
        # see the rarest class in its train split, e.g. select with n=6)
        ...
        print(f"fold {fold}: train={len(train_idx)} test={len(test_idx)} "
              f"train_class_dist={dict(Counter(y_train.tolist()))}", flush=True)

    print("\n=== Detective: real grouped(by-problem)/stratified(by-benchmark) 5-fold CV, out-of-fold ===")
    ...
    if macro_auroc is not None:
        print(f"macro-AUROC (out-of-fold): {macro_auroc:.4f}  "
              f"(G6: >=0.70 accept H2, 0.60-0.70 weak, <0.60 reject)")

    print("\nper-class AUROC (one-vs-rest, out-of-fold):")
    for i, c in enumerate(classes):
        support = int(y_bin[:, i].sum())
        if support == 0 or support == len(y):
            print(f"  {c}: undefined (support={support})")
            continue
        try:
            auc = roc_auc_score(y_bin[:, i], oof_probs[:, i])
            print(f"  {c}: AUROC={auc:.4f}  support={support}")
        except ValueError as e:
            print(f"  {c}: could not compute ({e}), support={support}")

    print("\nper-class precision/recall/support (out-of-fold argmax predictions):")
    ...
    print(f"trivial 'always predict majority class ({majority_class})' accuracy: {majority_acc:.4f}")
    print(f"Detective's real lift over the trivial floor: {100*(overall_acc-majority_acc):.2f}pp")
